...
const Process = ({ className = "" }) => {
  ...
  const processSteps = [
    ...
    },
    {
      ...
      content:
        "The creation process lasts over 4 to 8 weeks, with regular updates like process photos included. Note that complex designs may take longer.",
    },
    {
      number: "6",
  ...
  const goToNext = () => {
    setCurrentIndex((prevIndex) =>
      Math.min(prevIndex + 1, processSteps.length - 1)
    );
  };
        ...
        >
          {processSteps.map((step, index) => (
            <div
              key={step.number}
              className={`${
                showArrows ? "snap-center process-box min-w-[322px]" : ""
              }`}
            >
